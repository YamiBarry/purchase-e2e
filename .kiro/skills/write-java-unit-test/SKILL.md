...
写完测试后，**优先使用 fast-test 脚本验证**，比 mvn test 快 5 倍：

### Linux / WSL

```bash
# 前提：首次需全量编译一次（之后不需要）
mvn install -DskipTests -pl <模块名> -am -T 8

# 改完测试后，快速编译+运行（~5秒 vs mvn 的 17秒+）
.kiro/skills/write-java-unit-test/scripts/fast-test.sh <项目根目录> <模块名> <测试文件路径>

# 示例
.kiro/skills/write-java-unit-test/scripts/fast-test.sh \
  /home/lucky/code/yami/seller/sellerportal-service \
...
# 改完测试后，快速编译+运行（~3.5秒 vs mvn 的 17秒+）
.kiro\skills\write-java-unit-test\scripts\fast-test.ps1 <项目根目录> <模块名> <测试文件路径>

# 示例
.kiro\skills\write-java-unit-test\scripts\fast-test.ps1 `
  D:\Documents\code\yamibuy\central-3p\seller\sellerportal-service `
  central-sellerportal-service `
  src/test/groovy/com/yamibuy/central/sellerportal/service/ShipmentServiceTest.groovy
```

**原理**：绕过 Maven 生命周期，直接 `groovyc` 编译单个 .groovy 文件 → JUnit Platform 跑测试（和 IDEA 一样的方式）。Windows 版使用 pathing jar 绕过命令行长度限制。

**内置依赖**：`scripts/junit-platform-console-standalone-1.7.2.jar` 已内置，无需联网下载。

**回退方案**：如果 fast-test 脚本报错（如新增了依赖），重新跑一次 `mvn install -DskipTests -am` 刷新 classpath 缓存。
