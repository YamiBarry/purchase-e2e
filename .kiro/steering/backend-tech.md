...
**⚠️ `ConfigService` / `LangConfigService` 是 CMS 配置（从 Redis/DB 读），不是 Apollo 配置！**

读取 Apollo 配置的正确方式：

```java
// 方式1：@Value 注入（推荐，支持热更新）
@Value("${canada_notice_enabled:true}")
private boolean canadaNoticeEnabled;
...
**禁止用 `ConfigService.get()` 读 Apollo 配置** — 那是 CMS 后台配置，数据源不同。
